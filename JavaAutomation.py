    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            if response.status != 204:
                print(
                    f"Failed to send the embed to the webhook. HTTP status: {response.status}"
                )
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        embed = Embed(
            title="Error",
            description=" ```Only the owner can use such commands. ```",
            color=Colour.red(),
        )
        await ctx.send(embed=embed)
...
async def onlyfree(ctx, status: str):
    if status.lower() not in ["on", "off"]:
        embed = Embed(
            title="Error",
            description="```Please use !onlyfree on or !onlyfree off```",
...
async def speed(ctx, new_speed: str):
    try:
        ...
        embed = Embed(
            title=" ```The scan speed must be a number. ```",
            color=Colour.from_rgb(255, 0, 0),
        )
        await ctx.send(embed=embed)
        return
    ...
    embed = Embed(
        title="Success!",
        description=f"```New scan speed: {new_speed_str}```",
        color=Colour.from_rgb(255, 182, 193),
    )
    await ctx.send(embed=embed)
...
async def owners(ctx):
    ...
    # Create an embed with the specified color
    embed = discord.Embed(
        title="Current Owners", color=discord.Color.from_rgb(255, 182, 193)
    )
...
async def restart(ctx):
    try:
        ...
        embed = Embed(
            title="Error",
            description="An error occurred while trying to restart the bot: {}".format(
                str(e)
            ),
            color=Colour.red(),
        )
...
async def more(ctx):
    ...
    autorestart_status = (
        "Off"
        if autorestart_task is None or autorestart_task.cancelled()
        else f"{autorestart_minutes} minutes"
    )
    ...
    embed = discord.Embed(
        title="More about you:", color=discord.Color.from_rgb(255, 182, 193)
    )
    embed.add_field(name="Prefix:", value=prefix, inline=False)
    embed.add_field(
        name="Roblox main:",
        value=main_username if main_cookie_valid else "Invalid cookie",
        inline=False,
    )
    embed.add_field(
        name="Roblox alt:",
        value=details_username if details_cookie_valid else "Invalid cookie",
        inline=False,
    )
    ...
    embed.add_field(
        name="Watching:", value=watching if watching else "No items", inline=False
    )
...
async def autorestart(ctx, minutes: Union[int, str] = None):
    ...
    if minutes is None:
        if autorestart_task is not None and not autorestart_task.cancelled():
            ...
            embed.add_field(
                name="Minutes", value=f"Restarting every {autorestart_minutes} minutes."
            )
            await ctx.send(embed=embed)
        else:
            embed = Embed(
        ...
        if autorestart_task is not None and not autorestart_task.cancelled():
            ...
            autorestart_minutes = None
            embed = Embed(
                title="Autorestart Disabled", color=Colour.from_rgb(255, 182, 193)
            )
...
async def check(ctx, cookie_type: str):
    ...
    for cookie in cookies:
        ...
        if valid:
            ...
            embed = Embed(
                title="Cookie check result:", color=Colour.from_rgb(255, 182, 193)
            )
            ...
            embed = Embed(
                title="Cookie check result:",
                description="The {} cookie in your settings was invalid".format(
                    cookie_type
                ),
                color=Colour.red(),
            )
